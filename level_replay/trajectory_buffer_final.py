...
class TrajectoryBuffer(object):
    ...
    def insert(self, storage):
        ...
        #indicates when episodes are done
        done = ~(storage.masks.cpu() > 0)
        #indicates when episode is done bc of time limit
        cliffhanger = ~(storage.cliffhanger_masks.cpu() > 0)

        #check if this is ok, cuz some pretty long trajs end up not done

        #extract all done_steps where the episodes are not cliffhangers
        #cliffhanger and done is 0, indicated in timestep following final observation and action
        done_idx = torch.logical_and(done, ~cliffhanger)
        done_idx = np.roll(done_idx, -1, axis=0)[:-1,:]
        self.done_seeds = np.unique(level_seeds[done_idx])

        # collect samples for each seed across processes (order is not preserved)
        ...
        for seed in seeds:
            ...
            #remove final timestep observation which is not matched to last timestep's action
    ...
    def _calculate_wasserstein_distance(self, seed_eval, seeds_compare):
        ...
        #SHAPE of each traj array: (#timesteps, act/obs#dim)
        ...
        for seed_idx in range(len(seeds_compare)):
            ...
            W = ot.emd2(s_eval, s_compare, M, numItermax=300000)
    ...
    def calculate_wasserstein_distance(self, seeds, level_sampler):
        ...
        for seed in seeds:
            ...
            if len(seeds_compare_)==0:
                ...
                #num_steps=-1 to avoid division by 0 error
                level_sampler.update_diversity(seed, 0, True, -1)
                continue
            ...
            done = seed in self.done_seeds
            num_steps = self.action_buffer[seed].shape[0]
            level_sampler.update_diversity(seed, diversity, done, num_steps)